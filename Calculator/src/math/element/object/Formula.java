...
public class Formula {
  ...
  /**
   * constructor, requires a formula string calls a function to create a linked
   * list out of the formula string
   * 
   * @author André
   * @param aFormula
   * 
  ...
  private void toLinkedList() {
    ...
    while (iStartPosition < iLenOfString) {
      if ((MathUtil.IsNumber(pFormula.charAt(iStartPosition)))
          || (MathUtil.IsComma(pFormula.charAt(iStartPosition)))) {
        iEndPosition = iStartPosition + 1;
        while ((iEndPosition < iLenOfString)
            && ((MathUtil.IsNumber(pFormula.charAt(iEndPosition))) || (MathUtil.IsComma(pFormula
                .charAt(iEndPosition))))) {
