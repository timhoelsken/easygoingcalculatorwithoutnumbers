...
public class StringConvertTest extends TestCase {
  ...
  public void testTermToStandardString() throws Exception {
    ...
    assertTrue("The String has Variables separated by *", tmpConverter.termToStandardString(tmpString).equals(
        "2*a+a*b"));
  }
  ...
  public void testEverythingEspaciallyTheSyntax() {
     ...
     * //"5+6+9+9@7" -> Fehler //Prüfe Operatoren //folgende Kombos sind
     * unmöglich: //"5*sin*6" -> Fehler (Unär/Binär) "5+6*7-*2" -> Fehler
     * (binär/binär) "5+sincos()+9" -> Fehler(Unnär/unnär) //erlaubt
     * "9+sin(5+2)" -> Ok
     * 
     * //Auf einen unären Operator muss ein Klammerausdruck folgen "5*sin(6)+3" ->
     * ok "5*sin6+3" -> nicht ok //vor und hinter einem unären operator muss ein
     * binärer operator stehen, es sei denn er steht am anfang oder ende der
     * formel "5*sin(6)cos(3)" -> fehler "5*sin(6)*cos(3)" -> ok
     * 
     ...
     * Operator (mit Klammerausdruck), binärer operator.... "5sin(66)4cos(3)" ->
     * fehler "5*sin(66)*4*cos(3)" -> ok
     * 
     * //Zahlen prüfen //Zahl darf nicht an Zahl angegrenzt sein "5(5)" ->
     * fehler "5*(5)" -> ok //zahl darf nicht zwei oder mehr ., enthalten
     * "5,23.43*2.22.22+2312" -> fehler "5.3*(5,2)" -> ok
     * 
     * //Klammer bewertung -> ( = 1, ) = -1 //=> Summe muss immer 0 sein
     * "((((5+6+3+2))" -> Fehler "((((5+6+3+2))))" -> ok
     * 
     ...
     * stehen, rechts nur ein binärer "(5+6)" -> ok "4(5+6)" -> nicht ok
     * "sin(56)" -> ok "(5+6)sin(3)" -> fehler "5+6+(3+3)*1+2" -> ok
