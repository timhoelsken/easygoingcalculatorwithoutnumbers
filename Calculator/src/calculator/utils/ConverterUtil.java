...
public final class ConverterUtil {
   ...
   *             if any check fails
   */
  public static String formulaToStandardString(String aFormula) throws FormulaConversionException {
   ...
   * @return the standard formula
   */
  public static String formulaToGUIStandardString(String aFormula) {
    ...
    if (aFormula.contains(".")) {
      ...
      if (tmpCommaPosition != 0) {
        ...
        while (tmpCommaPosition != -1) {
          ...
          if (tmpCommaPosition + 1 >= aFormula.length()
              || !MathUtil.isNumber(aFormula.charAt(tmpCommaPosition + 1))) {
            throw new FormulaConversionException("The formula contains invalid commas.");
          }
  ...
  public static void checkOperators(String aFormula) throws FormulaConversionException {
    ...
    if (tmpMatcher.find()) {
      throw new FormulaConversionException(
          "Do not let an alone standing arithmetic operator follow an opening bracket.");
    }
  }
  ...
  public static String centerStringInSpaces(String someSpaces, String aString) {
    ...
    int tmpSpacesLength = someSpaces.length();
    int tmpStringLength = aString.length();
    // if spaces shorter or equal than the string...
    if (tmpSpacesLength <= tmpStringLength) {
      return aString;
    }
    // divide the left part of the spaces to start with the string there
    int tmpStart = (tmpSpacesLength - tmpStringLength) / 2;
